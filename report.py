...
COLORS = {"GRU": "#2563EB", "GNN": "#DC2626", "SARIMA": "#7C3AED", "Persistence": "#6B7280"}
METRIC_LABELS = {
    "MAE":       "MAE (kt)",
    "RMSE":      "RMSE (kt)",
    "nRMSE":     "nRMSE",
...
def plot_training_curves(data: dict[str, dict]) -> str:
    # Only plot models that have epoch-based training (len > 1)
    epoch_models = {name: r for name, r in data.items() if len(r["train_loss"]) > 1}
    if not epoch_models:
        fig, ax = plt.subplots(figsize=(5, 3))
        ax.text(0.5, 0.5, "No epoch-based training curves available",
                ha="center", va="center", transform=ax.transAxes)
        ax.axis("off")
        fig.tight_layout()
        return fig_to_b64(fig)
    fig, axes = plt.subplots(1, len(epoch_models), figsize=(6 * len(epoch_models), 4), squeeze=False)
    for ax, (name, r) in zip(axes[0], epoch_models.items()):
...
def plot_sarima_params(sarima_data: dict) -> str:
    """
    Bar chart of SARIMA parameters with 95 % confidence intervals,
    grouped by component.
    """
    params      = sarima_data["params"]
    names       = [str(n) for n in sarima_data["param_names"]]
    lower       = sarima_data["param_conf_lower"]
    upper       = sarima_data["param_conf_upper"]

    # Pretty names and component grouping
    COMPONENT_MAP = {
        "ar.L1":    ("AR(1)",             "Non-seasonal AR"),
        "ma.L1":    ("MA(1)",             "Non-seasonal MA"),
        "ar.S.L24": ("Seasonal AR(24)",   "Seasonal AR"),
        "ma.S.L24": ("Seasonal MA(24)",   "Seasonal MA"),
        "sigma2":   ("σ² (noise)",        "Noise variance"),
    }

    labels, values, err_lo, err_hi, colors = [], [], [], [], []
    component_palette = {
        "Non-seasonal AR": "#2563EB",
        "Non-seasonal MA": "#0891b2",
        "Seasonal AR":     "#7C3AED",
        "Seasonal MA":     "#9333ea",
        "Noise variance":  "#64748b",
    }

    for raw_name, val, lo, hi in zip(names, params, lower, upper):
        pretty, component = COMPONENT_MAP.get(raw_name, (raw_name, "Other"))
        labels.append(pretty)
        values.append(float(val))
        err_lo.append(float(val) - float(lo))
        err_hi.append(float(hi) - float(val))
        colors.append(component_palette.get(component, "#94a3b8"))
    ...
    ax.errorbar(
        x, values,
        yerr=[err_lo, err_hi],
        fmt="none", color="#1e293b", capsize=6, linewidth=1.5, zorder=4,
    )
    ax.axhline(0, color="#94a3b8", linewidth=0.8, linestyle="--", zorder=2)
    for xi, val in zip(x, values):
        ax.text(xi, val + (0.015 if val >= 0 else -0.045),
    ...
    ax.set_title(
        f"SARIMA(1,0,1)(1,1,1)[24]  —  Parameter Estimates with 95 % Confidence Intervals\n"
        f"AIC = {float(sarima_data['aic'][0]):.2f}   BIC = {float(sarima_data['bic'][0]):.2f}",
        fontsize=10,
    )
    ...
    # Legend for colours
    legend_handles = [
        mpatches.Patch(facecolor=v, label=k) for k, v in component_palette.items()
        if any(COMPONENT_MAP.get(n, ("", k))[1] == k for n in names)
    ]
...
def sarima_param_table_html(sarima_data: dict) -> str:
    """HTML table of SARIMA parameters with CIs and plain-English descriptions."""
    DESCRIPTIONS = {
        "ar.L1": (
            "AR(1) — Non-seasonal autoregression",
            "The kt value one hour ago explains this fraction of the current residual. "
            "Strong positive value (≈0.93) means kt is highly correlated with its immediate past "
            "(slow-changing cloud cover, inertia in atmospheric state).",
        ),
        "ma.L1": (
            "MA(1) — Non-seasonal moving average",
            "Corrects for the forecast error made one hour ago. "
            "Small negative value (≈−0.13) means a previous over-prediction is partially reversed.",
        ),
        "ar.S.L24": (
            "SAR(1)[24] — Seasonal autoregression",
            "The kt value exactly 24 hours ago (same hour yesterday) contributes to today's forecast. "
            "Negative value (≈−0.11) suggests a mild mean-reverting day-to-day pattern: "
            "a clear day is slightly more likely to be followed by a cloudier one.",
        ),
        "ma.S.L24": (
            "SMA(1)[24] — Seasonal moving average",
            "Large negative value (≈−0.96) captures the daily seasonal difference: "
            "after a one-period seasonal differencing, the model corrects strongly for "
            "yesterday's prediction error at the same hour.",
        ),
        "sigma2": (
            "σ² — Innovation variance",
            "Residual noise variance after all AR/MA components are accounted for. "
            "Small value (≈0.022 kt²) means the model captures most systematic structure.",
        ),
    }
    ...
    rows = ""
    for raw_name, val, lo, hi in zip(names, params, lower, upper):
        short_name, description = DESCRIPTIONS.get(raw_name, (raw_name, "—"))
        rows += (
            f"<tr><td><code>{raw_name}</code></td>"
            f"<td><strong>{short_name}</strong></td>"
            f"<td>{float(val):.4f}</td>"
            f"<td>[{float(lo):.4f}, {float(hi):.4f}]</td>"
            f"<td style='font-size:0.82rem;color:#475569;'>{description}</td></tr>"
        )
    return f"""
    <table>
      <caption>SARIMA(1,0,1)(1,1,1)[24] — Parameter Estimates</caption>
      <thead><tr>
        <th>Parameter</th><th>Component</th><th>Estimate</th><th>95% CI</th><th>Interpretation</th>
      </tr></thead>
      <tbody>{rows}</tbody>
    </table>"""
...
.badge-sarima   { background:#ede9fe; color:#6d28d9; margin-right:0.3rem; }
...
def build_html(
    ...
    # SARIMA section (optional)
    sarima_html = ""
    if sarima_data is not None:
        aic_val = float(sarima_data["aic"][0])
        bic_val = float(sarima_data["bic"][0])
        order   = sarima_data.get("order", [1, 0, 1])
        sorder  = sarima_data.get("seasonal_order", [1, 1, 1, 24])
        sarima_html = f"""
  <!-- ── SARIMA Interpretability ── -->
  <h2>SARIMA — Model Interpretability</h2>
  <div class="callout" style="background:#f5f3ff; border-color:#7c3aed;">
    <strong>Why include a SARIMA model?</strong><br>
    Unlike the GRU and GNN models (black boxes), SARIMA has fully interpretable parameters.
    Each coefficient has a concrete statistical meaning and comes with a 95&nbsp;% confidence interval.
    SARIMA serves as a transparent baseline that shows how much of the signal can be explained
    by linear temporal patterns alone — and quantifies the added value of deep learning.
  </div>

  <h3>Model specification: SARIMA({order[0]},{order[1]},{order[2]})({sorder[0]},{sorder[1]},{sorder[2]})[{sorder[3]}]</h3>
  <p style="font-size:0.9rem; color:#475569; margin-bottom:1rem;">
    <strong>Non-seasonal part (p=1, d=0, q=1):</strong>
    one autoregressive lag + one moving-average lag on the original (undifferenced) series.<br>
    <strong>Seasonal part (P=1, D=1, Q=1) with period s=24 h:</strong>
    one seasonal differencing (removes daily mean trend), one seasonal AR lag at 24&nbsp;h,
    one seasonal MA lag at 24&nbsp;h.<br>
    <strong>Fit on the last 8,760 hours (1 year) of training data</strong>
    to keep runtime manageable; parameters are applied to the full series via the Kalman filter.
    <strong>AIC = {aic_val:.2f} &nbsp;·&nbsp; BIC = {bic_val:.2f}</strong>
  </p>
  {"<div class='plot-box'>" + img_tag(plots["sarima_params"], "SARIMA parameters") + "</div>"
   if "sarima_params" in plots else ""}
  {sarima_param_table_html(sarima_data)}

  <h3>Forecast methodology</h3>
  <ul style="font-size:0.9rem; color:#475569; line-height:1.9; padding-left:1.4rem; margin-bottom:1.5rem;">
    <li><strong>1 h horizon:</strong> Kalman-filter one-step-ahead predictions —
        the model uses all observations up to time&nbsp;t and predicts t+1.
        This is the gold-standard use of a SARIMA model.</li>
    <li><strong>6 h &amp; 24 h horizons:</strong> daily-batch dynamic forecasting —
        the model is re-anchored to true observations every 24&nbsp;h (each new day),
        then runs recursively within that window.
        This limits error accumulation while staying realistic.</li>
  </ul>"""
    tables = "\n".join(
...
def main(args: argparse.Namespace) -> None:
    ...
    sarima_path = ROOT / "results_sarima.npz"

    if gru_path.exists() and not args.gnn_only and not args.sarima_only:
        r = load_results(gru_path)
        results_data["GRU"] = r
        print(f"[report] Loaded GRU results  ({len(r['y_true'])} test samples)")

    if gnn_path.exists() and not args.gru_only and not args.sarima_only:
        r = load_results(gnn_path)
        results_data["GNN"] = r
        print(f"[report] Loaded GNN results  ({len(r['y_true'])} test samples)")

    if sarima_path.exists() and not args.gru_only and not args.gnn_only:
        r = load_results(sarima_path)
        results_data["SARIMA"] = r
        sarima_data = r
        print(f"[report] Loaded SARIMA results ({len(r['y_true'])} test samples)")
    ...
    if sarima_data is not None:
        plots["sarima_params"] = plot_sarima_params(sarima_data)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sarima_only", action="store_true", help="Only include SARIMA results")
